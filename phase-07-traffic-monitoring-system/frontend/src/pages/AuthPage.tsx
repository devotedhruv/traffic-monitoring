...
import { Activity, ArrowLeft, ArrowRight, CheckCircle2, Eye, EyeOff, LockKeyhole, Mail, ScanLine, ShieldCheck, UserRound } from "lucide-react";
    ...
    <div className="min-h-screen bg-page">
      <header className="absolute inset-x-0 top-0 z-20">
        <div className="mx-auto flex h-[76px] max-w-[1240px] items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center gap-2.5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-primary text-white"><Activity size={20} /></span><strong className="text-sm">TrafficOps <span className="text-primary">AI</span></strong></Link>
          <ThemeToggle />
        </div>
      </header>
      <main className="grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
        <section className="relative hidden overflow-hidden bg-[#08130f] px-12 pb-14 pt-28 text-white lg:flex lg:flex-col">
          <div className="landing-grid-dark absolute inset-0 opacity-80" />
          <div className="absolute -left-20 top-20 h-80 w-80 rounded-full bg-emerald-500/15 blur-3xl" />
          ...
          <div className="relative mx-auto flex w-full max-w-[570px] flex-1 flex-col justify-center">
            <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.17em] text-emerald-300"><ScanLine size={15} />Intelligence in motion</p>
            <h1 className="mt-6 text-[46px] font-extrabold leading-[1.08] tracking-[-0.05em]">Your traffic operations,<br/><span className="text-emerald-400">clear and connected.</span></h1>
            ...
            <div className="mt-10 grid max-w-lg gap-3 sm:grid-cols-3">
              {[
                ...
                return <div key={String(title)} className="rounded-2xl border border-white/10 bg-white/[0.045] p-4"><ItemIcon size={18} className="text-emerald-400" /><strong className="mt-3 block text-[10px]">{String(title)}</strong><span className="mt-1 block text-[8px] text-white/40">{String(note)}</span></div>;
              })}
            </div>
          </div>
          ...
          <div className="w-full max-w-[430px]">
            ...
            {status === "authenticated" && user ? (
              ...
              <>
                ...
                <p className="mt-6 text-center text-xs text-muted">{isSignup ? "Already have an account?" : "New to TrafficOps?"} <Link to={isSignup ? "/sign-in" : "/sign-up"} className="font-bold text-primary hover:text-primary-hover">{isSignup ? "Sign in" : "Create an account"}</Link></p>
