...
export function DetectionFilters({ query, onChange }: { query: VehicleQuery; onChange: (next: VehicleQuery) => void }) {
  ...
  const clear = () => onChange({ page: 1, pageSize: query.pageSize, status: "", type: "", speed: "", violation: "", date: "", search: "", sort: "time_desc" });
  const chips = [
    ...
    query.violation && { key: "violation", text: `Violation: ${query.violation.replaceAll("_", " ")}` }
  ].filter(Boolean) as { key: keyof VehicleQuery; text: string }[];
  return (
    <div className="border-b border-border bg-surface p-3 sm:p-4">
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(220px,1.2fr)_145px_155px_145px_155px_135px_140px_auto]">
        ...
        <select aria-label="Filter by vehicle type" value={query.type ?? ""} onChange={(e) => update({ type: e.target.value as VehicleQuery["type"] })} className="field"><option value="">All vehicle types</option><option value="bicycle">Bicycle</option><option value="car">Car</option><option value="motorcycle">Motorcycle</option><option value="bus">Bus</option><option value="truck">Truck</option><option value="unknown">Unknown</option></select>
        ...
        <select aria-label="Filter by violation" value={query.violation ?? ""} onChange={(e) => update({ violation: e.target.value as VehicleQuery["violation"] })} className="field"><option value="">All violations</option><option value="OVERSPEED">Overspeed</option><option value="NO_HELMET">No helmet</option><option value="WRONG_LANE">Wrong lane</option><option value="WRONG_DIRECTION">Wrong direction</option></select>
